...
# "Perl 5 to 6" Lesson 11 - 进化后 Perl 5 中的操作符
...
这个重复操作 x 现在分开成二个操作符: x 是重复字符串, xx 是重复列表.
...
enum bit Bool ; my $value = $arbitrary_value but True; if $value { say "Yes, it's true"; # will be printed }



enum Day ('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'); if custom_get_date().Day == Day::Sat | Day::Sun { say "Weekend"; }
`描述`
