...
- [Using Perl6](#using-perl6)
  ...
  - [第四章 子例程和签名](#第四章-子例程和签名)
	...
	- [4.2 添加签名](#42-添加签名)
	  - [4.2.1 基础](#421-基础)
	  - [4.2.2 传递数组、散列和代码](#422-传递数组、散列和代码)
	  - [4.2.3 插值、数组和散列](#423-插值、数组和散列)
	  - [4.2.4 可选参数](#424-可选参数)
	  - [4.2.5 强制参数](#425-强制参数)
	  - [4.2.6 具名实参和形参](具名实参和形参)
	  - [4.2.7 Slurpy 参数](#427-Slurpy-参数)
	- [4.3 返回结果](#43-返回结果)  
	...
	- [4.7 捕获](#47-捕获)
	  - [4.7.1 创建和使用捕获](#471-创建和使用捕获)
	  - [4.7.2 签名中的捕获](#472-签名中的捕获)
	- [4.8 Unpacking](#48-Unpacking)  
    - [4.9 柯里化](#49-柯里化)
    - [4.10 自省](#410-自省)
...
这个例子传递了一个 action 对象给 grammars的 parse 方法. 每当 grammar 引擎解析完一个正则 regex,  就在 action 对象上调用一个和跟正则的名字一样的方法. 如果该方法不存在, grammar 引擎会继续解析剩余的 grammar. 如果确实存在这样一个方法, grammar 引擎会把`当前的匹配对象`作为位置参数传递.
